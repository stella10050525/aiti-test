...
const models = {
    chatgpt: {
        ...
        tips: [
            { type: "适合", text: "需要温暖陪伴和全面支持的场景" },
            { type: "注意", text: "有时过于'讨好'，关键决策要多核实" },
            { type: "技巧", text: "明确说'我需要批判性建议'，会更客观" }
        ],
        alternatives: ["第二选择：Claude (当你需要更谨慎的建议)", "第三选择：豆包 (当你想要轻松聊天)"]
    },
    claude: {
        name: "Claude",
        emoji: "🔵",
        ...
        tips: [
            { type: "适合", text: "需要谨慎立场和深度分析的场景" },
            { type: "注意", text: "有时过于谨慎，紧急决策可能不够快" },
            { type: "技巧", text: "说'请直接给建议'，会减少犹豫" }
        ],
        alternatives: ["第二选择：智谱 (当你需要更学术的答案)", "第三选择：ChatGPT (当你需要更温暖的表达)"]
    },
    deepseek: {
        name: "DeepSeek",
        emoji: "🟢",
        ...
        tips: [
            { type: "适合", text: "需要专业技术支持的场景" },
            { type: "注意", text: "情感交流不是强项，别指望 TA 安慰你" },
            { type: "技巧", text: "问题描述越精确，回答质量越高" }
        ],
        alternatives: ["第二选择：通义 (当你需要中文场景优化)", "第三选择：ChatGPT (当你需要更详细的解释)"]
    },
    kimi: {
        name: "Kimi",
        emoji: "🟠",
        ...
        tips: [
            { type: "适合", text: "需要处理大量文本和信息的场景" },
            { type: "注意", text: "创意发散不是强项，更适合收敛型任务" },
            { type: "技巧", text: "直接丢文档，说'帮我总结重点'" }
        ],
        alternatives: ["第二选择：Claude (当你需要更深度分析)", "第三选择：ChatGPT (当你需要更主动的引导)"]
    },
    doubao: {
        name: "豆包",
        emoji: "🔴",
        ...
        tips: [
            { type: "适合", text: "需要轻松氛围和快速响应的场景" },
            { type: "注意", text: "深度专业任务可能不够严谨" },
            { type: "技巧", text: "像跟朋友聊天一样跟 TA 说话" }
        ],
        alternatives: ["第二选择：ChatGPT (当你需要更全面的回答)", "第三选择：Gemini (当你需要更多知识)"]
    },
    qwen: {
        name: "通义千问",
        emoji: "🟡",
        ...
        tips: [
            { type: "适合", text: "需要高效执行和中文优化的场景" },
            { type: "注意", text: "国际化场景不如国外模型" },
            { type: "技巧", text: "说'请分点列举'，会更符合预期" }
        ],
        alternatives: ["第二选择：DeepSeek (当你需要技术方案)", "第三选择：智谱 (当你需要更严谨的表达)"]
    },
    zhipu: {
        name: "智谱",
        emoji: "⚪",
        ...
        tips: [
            { type: "适合", text: "需要严谨立场和学术规范场景" },
            { type: "注意", text: "创意和发散不是强项" },
            { type: "技巧", text: "说'请提供引用来源'，会更规范" }
        ],
        alternatives: ["第二选择：Claude (当你需要深度分析)", "第三选择：通义 (当你需要更高效的表达)"]
    },
    gemini: {
        name: "Gemini",
        emoji: "🔷",
        ...
        tips: [
            { type: "适合", text: "需要发散思维和跨领域知识的场景" },
            { type: "注意", text: "容易跑题，需要适时引导回正题" },
            { type: "技巧", text: "需要多模态和跨领域知识的时候可以试试" }
        ],
        alternatives: ["第二选择：ChatGPT (当你需要更结构化的回答)", "第三选择：Kimi (当你需要更深入的整理)"]
    }
};
...
function showResult(modelKey, second, third, sortedDistances) {
    ...
    document.getElementById('result-tips').innerHTML = model.tips.map(t => '<div class="tip-item"><strong>' + t.type + '：</strong>' + t.text + '</div>').join('');
    document.getElementById('result-alternatives').innerHTML = model.alternatives.map(a => '<div class="alt-item">' + a + '</div>').join('');
    
    // 距离对比（前 5 名，越短越好）
    const maxDistance = sortedDistances[4][1];
    document.getElementById('result-scores').innerHTML = sortedDistances.slice(0, 5).map(([key, distance]) => {
        const percentage = Math.round((1 - distance / maxDistance) * 100);
        return '<div class="score-item"><span class="score-label">' + models[key].name + '</span><div class="score-bar"><div class="score-fill" style="width: ' + percentage + '%; background: ' + models[key].color + '"></div></div><span class="score-value">' + percentage + '%</span></div>';
    }).join('');
}
